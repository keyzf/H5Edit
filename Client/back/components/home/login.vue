			...
			login: function(){
				...
				$.ajax({
					...
					success:function(data){
						if(data.iserro){
							self.erro.iserro = true;
							self.erro.text = data.msg;
							self.erro.target = data.data.target;
							return;
						}
						localStorage.userInfo = JSON.stringify(data.data);
						if(self.remember){
							localStorage.remember = self.remember;
						}else{
							localStorage.remember = false;
						}
						self.$route.router.go('/list');
					}
				});
			}
		}
