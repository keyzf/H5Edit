<template>
	<div class="side_con right" v-show="checkedItems.length == 1" transition="fromRight">
		<div class="item attrList">
			...
			<div class="item_id">元素ID为 : <span>{{checkedItemDataOnlyOneId}}</span></div>
			<div class="main">
	...
	module.exports = {
		...
		vuex: {
			getters: {
				...
				checkedItemData: function(state) {
					return state.workData.mainCode.pages[state.currentPage - 1].items[state.checkedItems[0]];
				},
				checkedItemDataOnlyOneId: function(state) {
					return state.checkedItems.length == 1 ? state.workData.mainCode.pages[state.currentPage - 1].items[state.checkedItems[0]].id : '';
				}
			},
			actions: {
		...
		methods: {
			setStyleDirect: function(ev, type, px){ // px 单位
				...
				this.setStyle(this.checkedItems[0], params,true)
				this.style = utils.getStyle(this.checkedItemData,'all',true);
			},
			setBoxShadow: function(ev,type,px){
				...
				this.setStyle(this.checkedItems[0], {
			...
			addAni: function(){
				this.addClass(this.checkedItems[0],'ani');
				var model = 'none 0s ease 0s 1 none';
				var _aniStr = utils.getStyle(this.checkedItemData,'-webkit-animation',true) || utils.getStyle(this.checkedItemData,'animation',true);
				if(_aniStr !== null){
					this.setStyle(this.checkedItems[0],{
				...
				var aniStr = utils.getStyle(this.checkedItemData,'-webkit-animation',true) || utils.getStyle(this.checkedItemData,'animation',true);
			...
			delAni: function(index){
				...
				if(resultStr.length === 0){
					this.removeClass(this.checkedItems[0],'ani');
				}
				this.setStyle(this.checkedItems[0],{
			...
			reloadAni: function(){
				...
				setTimeout(function(){
					self.setStyle(self.checkedItems[0],{
						'animation': _AniStr,
			...
			'checkedItems': {
				handler: function(value){
					...
					if(this.checkedItems[0] !== undefined){
						...
						this.style = utils.getStyle(this.checkedItemData,'all',true);
						this.boxShadowStyle = this.formatBoxShadow(utils.getStyle(this.checkedItemData,'box-shadow',true));
						var aniStr = utils.getStyle(this.checkedItemData,'-webkit-animation',true) || utils.getStyle(this.checkedItemData,'animation',true);
...
<style>
	...
	.nav_top_btn li.active {
		background: #01d7b2;
		color: #fff;
	}
	.nav_top_btn li:hover {
		background: #01d7b2;
		color: #fff;
	}
	.attr_main {
	...
	.group_main input[type="text"]:focus {
		border: 1px solid #01d7b2;
	}
	.group_main input[type="number"]:focus {
		border: 1px solid #01d7b2;
	}
	.group_main input[type="color"] {
		width: 40px;
		margin-right: 20px;
		border: 1px solid #ccd5db;
	}
	.group_main input[type="text"] {
		width: 85px;
		border: 1px solid #ccd5db;
		height: 20px;
		line-height: 20px;
	...
	.group_main input[type="range"] {
		width: 85px;
		margin-right: 20px;
	}
	.group_main input[type="number"] {
		width: 40px;
		border: 1px solid #ccd5db;
		height: 20px;
		line-height: 20px;
	...
	.group_main input[type="number"]:disabled {
		cursor: not-allowed;
	}
	.group_main select {
		display: inline-block;
	...
	.ani_btn li:first-child {
		background: rgba(1,215,178,1);
	}
	.ani_btn li:first-child:hover {
		background: rgba(1,215,178,0.7);
	}
	.ani_btn li:last-child {
		background: rgba(8,161,239,1);
	}
	.ani_btn li:last-child:hover {
		background: rgba(8,161,239,0.7);
	}
	.item_id {
		font-size: 14px;
		line-height: 28px;
